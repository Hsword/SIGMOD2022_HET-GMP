...
void getNcclUniqueId(ncclUniqueId* Id, MPI_Comm mpi_comm, int localRank, int senderRank){
...
void _ncclBroadcast(const void* sendbuff, void* recvbuff, int size, int datatype,
                    int root, ncclComm_t comm, cudaStream_t stream){
  NCCLCHECK(ncclBroadcast((const void *)sendbuff, (void* )recvbuff, size, _get_proper_datatype(datatype), root, comm, stream));
}

void _ncclAllGather(const void* sendbuff, void* recvbuff, int size, int datatype,
                    ncclComm_t comm, cudaStream_t stream){
  NCCLCHECK(ncclAllGather((const void*)sendbuff, (void*)recvbuff, size, _get_proper_datatype(datatype), comm, stream));
}

void _ncclSend(const void* sendbuff, int size, int datatype, int target, ncclComm_t comm, cudaStream_t stream){
  NCCLCHECK(ncclSend(sendbuff, size, _get_proper_datatype(datatype), target, comm, stream));
}

void _ncclRecv(void* recvbuff, int size, int datatype, int src, ncclComm_t comm, cudaStream_t stream){
  NCCLCHECK(ncclRecv(recvbuff, size, _get_proper_datatype(datatype), src, comm, stream));
}

void dlarrayAllReduce(DLArray *input_array,  DLArray *output_array, int datatype, int op,
  ...
  _ncclAllReduce(input_data_buffer, output_data_buffer, size, datatype, op, comm, stream);
}

void dlarrayBroadcast(DLArray *input_array,  DLArray *output_array, int datatype, int root,
                      ncclComm_t comm, DLStreamHandle stream_handle){
    int size = 1;
    for(int i = 0; i < input_array->ndim; i++){
      size = size * input_array->shape[i];
    ...
    float *output_data_buffer = (float*)(output_array->data);
    cudaStream_t stream = *(cudaStream_t*)stream_handle->handle;
    _ncclBroadcast(input_data_buffer, output_data_buffer, size, datatype, root, comm, stream);                        
}

void dlarrayAllGather(DLArray *array, DLArray *output_array, int datatype, ncclComm_t comm, DLStreamHandle stream_handle){
  int size = 1;
...
void dlarraySend(DLArray *array, int datatype, int target, ncclComm_t comm, DLStreamHandle stream_handle){
  int size = 1;
  for(int i = 0; i < array->ndim; i++){
    size = size * array->shape[i];
  }
...
void dlarrayRecv(DLArray *array, int datatype, int src, ncclComm_t comm, DLStreamHandle stream_handle){
  int size = 1;
  for(int i = 0; i < array->ndim; i++){
    size = size * array->shape[i];
  }
