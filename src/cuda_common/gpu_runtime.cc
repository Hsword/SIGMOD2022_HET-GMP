...
void cudnn_init(size_t dev_id, DLStreamHandle stream){
  ...
  if(stream){
      CUDNN_CALL(cudnnSetStream(cudnn_map[dev_id], *(cudaStream_t*)stream->handle));
  }
}
...
void cusp_init(size_t dev_id, DLStreamHandle stream){
  ...
  if(stream){
      CUSP_CALL(cusparseSetStream(cusp_map[dev_id], *(cudaStream_t*)stream->handle));
  }
}
...
void cublas_init(size_t dev_id, DLStreamHandle stream){
  ...
  if(stream){
      CUBLAS_CALL(cublasSetStream(cublas_map[dev_id], *(cudaStream_t*)stream->handle));
  }
}
...
int DLEventRecord(DLStreamHandle stream_handle, DLEventHandle event_handle){
  CUDA_CALL(cudaEventRecord(*(cudaEvent_t*)event_handle->handle, *(cudaStream_t*)stream_handle->handle));
  return 0;
}
