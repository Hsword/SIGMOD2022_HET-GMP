...
bool is_chunk_init(size_t dev_id){
  if (init_free_chunk_set.empty() || init_free_chunk_set.find(dev_id) == init_free_chunk_set.end()){
    return false;
  }
  else{
    //assert(init_free_chunk_set.find(dev_id) != init_free_chunk_set.end());
    return init_free_chunk_set[dev_id] == 1;
  }
}

void chunk_init(size_t dev_id){
  bool is_chunk_empty = init_free_chunk_set.find(dev_id) == init_free_chunk_set.end();
  if (is_chunk_empty)
  {
    init_free_chunk_set.insert(std::pair<int, bool>(dev_id, false));
    free_chunk_set.insert(std::pair<int, std::multiset<Chunk>>(dev_id, std::multiset<Chunk>()));
    all_malloced_chunk.insert(std::pair<int, std::map<void*, size_t>>(dev_id, std::map<void*, size_t>()));
  }
...
void* find_chunk(size_t _chunk_size, size_t dev_id){
  ...
  if((it == free_chunk_set[dev_id].end()) || (it->chunk_size != _chunk_size)){
    // std::cout<<work_data<<std::endl;
    void *work_data = NULL;
    cudaSetDevice(dev_id);
    DebugCudaMalloc(cudaMalloc(&work_data, _chunk_size));
    all_malloced_chunk[dev_id].insert(std::pair<void*, size_t>(work_data, _chunk_size));
    // std::cout<<work_data<<std::endl;
    return work_data;
  }
  else{
