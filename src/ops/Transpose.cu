...
int DLGpuTranspose(const DLArrayHandle input, DLArrayHandle output, int *perm, DLStreamHandle stream_handle = NULL){
    ...
    if (cu_stream != NULL) {
        CUDA_CALL(cudaMemcpyAsync(gpu_buf, (void*)buf, buf_size, cudaMemcpyHostToDevice, cu_stream));
        transpose_kernel<<<blocks, threads, 0, cu_stream>>>(output_data, input_data, gpu_buf, ndim, size);
    } else {
...
int DLGpuTransposeSimple(const DLArrayHandle input, DLArrayHandle output, const DLArrayHandle gpu_buffer, DLStreamHandle stream_handle = NULL){
