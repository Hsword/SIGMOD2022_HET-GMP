...
int DLGpuBroadcastShape(const DLArrayHandle in_arr, DLArrayHandle out_arr, int* add_axes, DLStreamHandle stream_handle = NULL) {
    ...
    if (cu_stream != NULL) {
        CUDA_CALL(cudaMemcpyAsync(gpu_strides, out_strides, allocated, cudaMemcpyHostToDevice, cu_stream));
        CUDA_CALL(cudaMemcpyAsync(gpu_dims, in_dims, allocated, cudaMemcpyHostToDevice, cu_stream));
...
int DLGpuBroadcastShapeSimple(const DLArrayHandle in_arr, DLArrayHandle out_arr, 
    ...
    if (cu_stream != NULL) {
        broadcast_shape_kernel<<<blocks, threads, 0, cu_stream>>>(
            in_data, out_data, os_data, id_data, (size_t)out_arr->ndim, output_size);
    } else {
        broadcast_shape_kernel<<<blocks, threads>>>(
            in_data, out_data, os_data, id_data, (size_t)out_arr->ndim, output_size);
    }
