...
__global__ void ele_mult_kernel_2(const float *matA, const float *matB, float *output, size_t size, size_t size_2) {
...
int DLGpuDot(const DLArrayHandle matA, const DLArrayHandle matB, 
    ...
    if (stream_handle)
      ele_mult_kernel_2<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(matA_data, matB_data, output_data, size_A, size_B);
    else
      ele_mult_kernel_2<<<blocks, threads>>>(matA_data, matB_data, output_data, size_A, size_B);
    return 0;
