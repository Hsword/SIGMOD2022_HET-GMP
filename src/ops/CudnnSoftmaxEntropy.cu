...
__global__ void cudnn_cross_entropy_kernel(const float *logsoftmax, const float *label, float *output, size_t size) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
    output[idx] = -logsoftmax[idx] * label[idx];
}

int CuDNN_DLGpuSoftmaxEntropy(const DLArrayHandle input_y, const DLArrayHandle label, DLArrayHandle output, DLStreamHandle stream_handle = NULL) {
    ...
    CUDNN_CALL(cudnnSoftmaxForward(cudnn_map[dev_id], CUDNN_SOFTMAX_LOG, CUDNN_SOFTMAX_MODE_INSTANCE, 
    ...
    if (stream_handle) {
        cudnn_cross_entropy_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>
    ...
    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, temp_data, size * sizeof(float),
        &alpha, desc, (const void*)temp_data, &beta, new_desc, (void*)output_data));
...
__global__ void cudnn_softmax_cross_entropy_gradient(
    const float *pred, const float *y_, const float *grad_data, float *output_data, int last_dim, size_t size) {
...
int CuDNN_DLGpuSoftmaxEntropyGradient(const DLArrayHandle grad, const DLArrayHandle input_y, 
    const DLArrayHandle label, DLArrayHandle output, DLStreamHandle stream_handle = NULL) {
    size_t indim = input_y->ndim;
    assert (indim == label->ndim && indim == output->ndim && indim == grad->ndim + 1);
    int dev_id = (input_y->ctx).device_id;
    cudnn_init(dev_id, stream_handle);
    int n_ = 1;
    for (int i = 0; i < indim-1; ++i) {
    ...
    CUDNN_CALL(cudnnSetTensor4dDescriptor(desc, CUDNN_TENSOR_NCHW, 
            CUDNN_DATA_FLOAT, n_, c_, 1, 1));
    CUDNN_CALL(cudnnSoftmaxForward(cudnn_map[dev_id], CUDNN_SOFTMAX_ACCURATE, CUDNN_SOFTMAX_MODE_INSTANCE, 
        &alpha, desc, (const void*)y_data, &beta, desc, temp_data));
    dim3 blocks;
    dim3 threads;
    if (size <= 1024) {
    ...
    if (stream_handle) {
        cudnn_softmax_cross_entropy_gradient<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>
            ((const float*)temp_data, label_data, grad_data, output_data, c_, size);
    } else {
