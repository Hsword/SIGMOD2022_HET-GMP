...
__global__ void binary_cross_entropy_kernel(int nrow, const float *prediction,
    ...
    if (id >= nrow) return;
    loss[id] = - label[id] * log(prediction[id]) - (1 - label[id]) * log(1 - prediction[id]);
}

int DLGpuBinaryCrossEntropy(const DLArrayHandle prediction,
                             const DLArrayHandle label,
                             DLArrayHandle loss, DLStreamHandle stream_handle = NULL) {
    ...
    // 1 block
    if (stream_handle) {
      binary_cross_entropy_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
...
__global__ void binary_cross_entropy_gradient_kernel(int nrow,
    ...
    size_t id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= nrow) return;
    output[id] = output_grad[id] * (-label[id]/prediction[id] + (1 - label[id])/(1-prediction[id]));
}
...
int DLGpuBinaryCrossEntropy_Gradient(const DLArrayHandle prediction, const DLArrayHandle label,
    ...
    size_t indim = prediction->ndim;
    assert (indim >= 2 && indim == label->ndim && indim == output_grad->ndim && indim == output->ndim);
    int nrow = 1;
    for (int i = 0; i < indim-1; ++i) {
        nrow *= prediction->shape[i];
    }
    ...
    if (stream_handle) {
        binary_cross_entropy_gradient_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
