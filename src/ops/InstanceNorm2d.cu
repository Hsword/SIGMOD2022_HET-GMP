...
__global__ void minus_mean_n_square_kernel1(const float *in_arr, const float *mean, 
        float *out_arr, int last_2dim, size_t size) {
...
__global__ void std_normal_transform(const float *in_arr, const float *mean_arr, 
        const float *var_arr, float *out_arr, int last_2dim, float eps, size_t size) {
    size_t ind = blockIdx.x * blockDim.x + threadIdx.x;
    if (ind >= size) return;
...
int DLGpuInstanceNormalization2d(const DLArrayHandle in_arr, DLArrayHandle mean_arr,
    ...
    if (stream_handle)
        minus_mean_n_square_kernel1<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
            (const float*)in_arr->data, (const float*)mean_arr->data, (float*)out_arr->data, last_2dim, temp_strideA);
    else
    ...
    if (stream_handle)
        std_normal_transform<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
            (const float*)in_arr->data, (const float*)mean_arr->data, 
            (const float*)var_arr->data, (float*)out_arr->data, last_2dim, eps, temp_strideA);
    else
...
__global__ void calculate_grad_kernel(const float* out_grads, const float* in_arr,
        const float* mean_arr, const float* var_arr, float* grad_arr, size_t last2dim,
        float eps, size_t size) {
    size_t ind = blockIdx.x * blockDim.x + threadIdx.x;
    if (ind >= size) return;
    size_t mo_ind = ind / last2dim;
    float y = (in_arr[ind] - mean_arr[mo_ind]) / sqrtf(var_arr[mo_ind] + eps);
    grad_arr[ind] = out_grads[ind] * (1.0 - 1.0 / (float)last2dim - y * y) / sqrtf(var_arr[mo_ind] + eps);
}
...
int DLGpuInstanceNormalization2dGradient(const DLArrayHandle out_grads, const DLArrayHandle in_arr,
    ...
    for (int i = 0; i < ndim; ++i) total_elements *= out_grads->shape[i];
    ...
    if (stream_handle)
        calculate_grad_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
