...
__global__ void minus_mean_n_square_kernel(const float *in_arr, const float *mean, 
...
__global__ void rescale_kernel(const float *in_arr, const float *mean_arr, 
        ...
        const float *ln_bias, float *out_arr, int last_dim, float eps, size_t size) {
    size_t ind = blockIdx.x * blockDim.x + threadIdx.x;
    if (ind >= size) return;
    size_t mo_ind = ind / last_dim;
    size_t ln_ind = ind % last_dim;
    out_arr[ind] = (in_arr[ind] - mean_arr[mo_ind]) / sqrtf(var_arr[mo_ind] + eps) * 
        ln_scale[ln_ind] + ln_bias[ln_ind];
}
...
int DLGpuLayerNormalization(const DLArrayHandle in_arr, const DLArrayHandle ln_scale, 
    ...
    CUDNN_CALL(cudnnSetTensorNdDescriptor(adesc, CUDNN_DATA_FLOAT, ndim, dimA, strideA));
    CUDNN_CALL(cudnnSetTensorNdDescriptor(cdesc, CUDNN_DATA_FLOAT, ndim, dimC, strideC));

    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, (void*)out_arr->data, size,
        &one, adesc, (const void*)in_arr->data, &zero, cdesc, (void*)mean_arr->data));
    ...
    if (stream_handle)
        minus_mean_n_square_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
            (const float*)in_arr->data, (const float*)mean_arr->data, (float*)out_arr->data, last_dim, temp_strideA);
    else
        minus_mean_n_square_kernel<<<blocks, threads>>>(
            (const float*)in_arr->data, (const float*)mean_arr->data, (float*)out_arr->data, last_dim, temp_strideA);

    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, (void*)out_arr->data, size,
        &one, adesc, (const void*)out_arr->data, &zero, cdesc, (void*)var_arr->data));
    ...
    if (stream_handle)
        rescale_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
            (const float*)in_arr->data, (const float*)mean_arr->data, 
            (const float*)var_arr->data, (const float*)ln_scale->data, 
            (const float*)ln_bias->data, (float*)out_arr->data, last_dim, eps, temp_strideA);
    else
...
__global__ void process_kernel1(const float *grads, const float *in_arr, 
        const float *mean_arr, const float *var_arr, const float *ln_scale, 
        float *ws1, float *ws2, float *out_arr, 
        float eps, int last_dim, size_t size) {
    size_t ind = blockIdx.x * blockDim.x + threadIdx.x;
    if (ind >= size) return;
...
int DLGpuLayerNormalizationGradient(const DLArrayHandle out_grads, const DLArrayHandle in_arr,
    ...
    CUDNN_CALL(cudnnSetTensorNdDescriptor(adesc, CUDNN_DATA_FLOAT, ndim, dimA, strideA));
    CUDNN_CALL(cudnnSetTensorNdDescriptor(cdesc, CUDNN_DATA_FLOAT, ndim, dimC, strideC));
    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, (void*)grad_arr->data, size,
        &one, adesc, (const void*)out_grads->data, &zero, cdesc, (void*)grad_bias->data));
    ...
    if (stream_handle)
        process_kernel1<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
    ...
    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, (void*)ws1, size,
        &one, adesc, (const void*)ws1, &zero, cdesc, (void*)grad_scale->data));
    ...
    CUDNN_CALL(cudnnSetTensorNdDescriptor(cdesc, CUDNN_DATA_FLOAT, ndim, dimC, strideC));
    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, (void*)grad_arr->data, size,
        &one, adesc, (const void*)grad_arr->data, &zero, cdesc, (void*)ws3));
    
    if (stream_handle)
        process_kernel2<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
            (const float*)out_grads->data, (const float*)in_arr->data, 
            (const float*)mean_arr->data, (const float*)var_arr->data, 
            (const float*)ln_scale->data, ws1, ws2, ws3,
            eps, last_dim, temp_strideA);
    else
    ...
    CUDNN_CALL(cudnnReduceTensor(cudnn_map[dev_id], rtd, NULL, 0, (void*)grad_arr->data, size,
        &one, adesc, (const void*)ws1, &zero, cdesc, (void*)ws3));
    
    if (stream_handle)
        process_kernel3<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
            ws1, ws3, (float*)grad_arr->data, last_dim, temp_strideA);
    else
        process_kernel3<<<blocks, threads>>>(
            ws1, ws3, (float*)grad_arr->data, last_dim, temp_strideA);
