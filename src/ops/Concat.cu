...
__global__ void concat_kernel(const float *x_data, const float *y_data, float *output_data, 
        int concat_size, int offset1, int offset2, size_t size) {
    ...
    if (mid_ind < offset1) {
        ...
        int y_ind = (pre_ind * offset2 + mid_ind - offset1) * concat_size + post_ind;
        val = y_data[y_ind];
    }
    output_data[ind] = val;
}
...
int DLGpuConcat(const DLArrayHandle input_x, const DLArrayHandle input_y, DLArrayHandle output, int axis=0, DLStreamHandle stream_handle=NULL) {
    ...
    if (stream_handle) {
        concat_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(x_data, y_data, output_data, 
...
__global__ void concat_gradient_kernel(const float *o_data, float *i_data,
        int concat_size, int concat_offset, int small_offset, int big_offset, size_t size) {
    size_t ind = blockIdx.x * blockDim.x + threadIdx.x;
    if (ind >= size) return;
    int post_ind = ind % concat_size;
    int temp = ind / concat_size;
...
int DLGpuConcat_gradient(const DLArrayHandle o_grad, DLArrayHandle i_grad, int axis=0, int id=0, DLStreamHandle stream_handle=NULL) {
    ...
    if (stream_handle) {
        concat_gradient_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(o_data, i_data, 
            concat_size, concat_offset, small_offset, big_offset, size);
    } else {
        concat_gradient_kernel<<<blocks, threads>>>(o_data, i_data,
            concat_size, concat_offset, small_offset, big_offset, size);
    }
    return 0;
}
