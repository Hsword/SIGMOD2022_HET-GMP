...
__global__ void softmax_cross_entropy_gradient_kernel(int nrow, int ncol, const float *input_a, const float *input_b, const float *input_c, float *output) {
    size_t id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= nrow) return;
    float maxval = input_a[id * ncol];
    // Find max for a row.
    ...
    for (int x = 0; x < ncol; ++x) {
        size_t curid = id * ncol + x;
        output[curid] = (exp(input_a[curid] - maxval) / sum - input_b[curid]) * input_c[id];
    }
}
...
int DLGpuSoftmaxCrossEntropy_Gradient(const DLArrayHandle input_a, const DLArrayHandle input_b,
    ...
    size_t indim = input_a->ndim;
    assert (indim >= 2 && indim == input_b->ndim && indim == input_c->ndim + 1 && indim == output->ndim);
    int nrow = 1;
    for (int i = 0; i < indim-1; ++i) {
        assert (input_a->shape[i] == input_b->shape[i] &&
            input_a->shape[i] == output->shape[i] &&
