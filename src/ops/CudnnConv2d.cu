...
int CuDNN_DLGpuConv2d(const DLArrayHandle input_x, const DLArrayHandle input_f, DLArrayHandle output,const int padding, const int stride, DLStreamHandle stream_handle = NULL){
    // cuda_init();
    // size_t now_device_id = (input_x->ctx).device_id;
    ...
    // std::cout<<"conv2d forward algo:"<< algo <<std::endl;
    ...
    // CUDA_CALL(cudaMalloc(&work_data, workspace_size));
    
    
    // std::cout << "device id " << devicedChoosed << std::endl;
    // std::cout<<"malloc ptr:    "<<work_data<<"   malloc size    "<<workspace_size<<std::endl;
    float alpha = 1.0f;
    float beta = 0.0f;
...
int CuDNN_DLGpuConv2d_Gradient_of_Filter(const DLArrayHandle input_x, const DLArrayHandle gradient_y, DLArrayHandle gradient_f, const int padding, const int stride, DLStreamHandle stream_handle = NULL){
    // cuda_init();
    // size_t now_device_id = (input_x->ctx).device_id;
    // cudaSetDevice(now_device_id);
    // create handle
...
int CuDNN_DLGpuConv2d_Gradient_of_Data(const DLArrayHandle input_f, const DLArrayHandle gradient_y, DLArrayHandle gradient_x,const int padding, const int stride, DLStreamHandle stream_handle = NULL){
  // cuda_init();
  // size_t now_device_id = (input_f->ctx).device_id;
  // cudaSetDevice(now_device_id);
  // create handle
  // CUDNN_CALL(cudnnCreate(&cudnn));
  ...
  // std::cout<<"conv2d backward data: "<<algo<<std::endl;
