...
__global__ void max_pool_forward_kernel(
    const int nthreads, const float *input_data,
  ...
  int DLGpuMax_Pooling2d(const DLArrayHandle input, 
            const int kernel_H, const int kernel_W,
            DLArrayHandle output, 
            const int padding, const int stride, DLStreamHandle stream_handle = NULL){
  ...
  int output_size = input_N * input_C * pooled_H * pooled_W;
    ...
    if (stream_handle)
      max_pool_forward_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
      output_size, input_data, input_H, input_W, input_C,
      pooled_H, pooled_W, kernel_H, kernel_W,
      stride, padding, output_data);
    else
  ...
  const int kernel_H, const int kernel_W,
    const int stride, const int padding, 
  const float *input_grad_data, float *output_grad_data) {
    const float kMinFLOAT8X4 = -FLT_MAX;
  ...
  int DLGpuMax_Pooling2d_gradient(const DLArrayHandle input, const DLArrayHandle input_grad,
            const int kernel_H, const int kernel_W, 
    ...
    if (stream_handle)
      max_pool_backward_kernel<<<blocks, threads, 0, *(cudaStream_t*)stream_handle->handle>>>(
      input_grad_size, input_data, input_H, input_W, input_C,
      pooled_H, pooled_W, kernel_H, kernel_W,
      stride, padding, input_grad_data, output_grad_data);
    else
