...
int DLGpuSlice(
    ...
    if (cu_stream != NULL) {
        CUDA_CALL(cudaMemcpyAsync(pos, (void *)begin_pos, 
            alloc_size, cudaMemcpyHostToDevice, cu_stream));
        CUDA_CALL(cudaMemcpyAsync(i_shape, (void *)in_arr->shape, 
            alloc_size, cudaMemcpyHostToDevice, cu_stream));
        CUDA_CALL(cudaMemcpyAsync(o_shape, (void *)out_arr->shape,
            alloc_size, cudaMemcpyHostToDevice, cu_stream));
...
__global__ void slice_kernel_simple(
    float *out_arr, 
    const float *in_arr, 
    const uint *gpu_buf,
    size_t ndim, 
...
int DLGpuSliceSimple(
    ...
    if (stream_handle) {
        ...
        slice_kernel_simple<<<blocks, threads, 0, cu_stream>>>
            ((float*)(out_arr->data), (const float*)(in_arr->data), (const uint*)(gpu_buf->data), ndim, o_size);
...
__global__ void slice_gradient_kernel_simple(
    float *out_arr, 
    const float *in_arr, 
    const uint *gpu_buf,
    size_t ndim, 
