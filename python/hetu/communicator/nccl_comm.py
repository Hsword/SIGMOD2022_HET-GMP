...
class NCCL_Communicator():
    ...
    def _create_streams(self):
        #lib_nccl.create_streams(self.streams, self.devs, self.devs_number)
        for i in range(self.devs_number.value):
            self.stream_handles.append(create_stream_handle(ndarray.gpu(i)))
            lib_nccl.update_stream(i, self.streams, c_int64(self.stream_handles[-1].handle.contents.handle))
    ...
    def _allreduce(self, send_buff, recv_buff, size):
        lib_nccl.NCCL_AllReduce(send_buff, recv_buff, size, self.comms, self.streams, self.devs_number)

    def get_send_buff(self, array):
        # for i in range(self.devs_number.value):
        #     self.send_buff[i] = array[i]
        self.send_buff = (c_void_p * self.devs_number.value)(*array)

    def get_recv_buff(self, array):
        # for i in range(self.devs_number.value):
        #     self.recv_buff[i] = array[i]  #TODO
    ...
    def All_Reduce_Ndarray(self, gradient_list, allreduced_gradient_list):
        ...
        for i in range(self.devs_number.value):
            ...
            allreduced_gradient_buff.append(allreduced_gradient_list[i].handle.contents.data)
        length = 1
