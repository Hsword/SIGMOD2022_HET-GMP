...
def cifar100(directory='CIFAR_100', onehot=True):
    ...
    train_images = np.array(train_images, dtype='float').reshape(train_images.shape[0], 3, 32, 32)
    ...
    test_images = np.array(test_images, dtype='float').reshape(test_images.shape[0], 3, 32, 32)
...
def normalize_cifar10(onehot=True):
...
def tf_normalize_cifar10(onehot=True):
    x_train, y_train, x_test, y_test = cifar10(onehot=onehot)
    x_train = x_train.reshape((-1, 3, 32, 32))
    x_test = x_test.reshape((-1, 3, 32, 32))
