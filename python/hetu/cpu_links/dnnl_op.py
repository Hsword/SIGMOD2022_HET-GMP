...
def matrix_multiply(matA, transposeA, matB, transposeB, matC):
    ...
    _LIB.DnnlMatrixMultiply(matA.handle, transposeA, matB.handle, transposeB, matC.handle)
...
def matrix_elementwise_multiply_by_const(mat, val, output):
    ...
    _LIB.DnnlMatrixElementwiseMultiplyByConst(mat.handle, ctypes.c_float(val), output.handle)
...
def matrix_elementwise_add_by_const(mat, val, output):
    assert isinstance(mat, _nd.NDArray)
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlMatrixElementwiseAddByConst(mat.handle, ctypes.c_float(val), output.handle)
...
def matrix_elementwise_divide_by_const(mat, val, output):
    assert isinstance(mat, _nd.NDArray)
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlMatrixElementwiseDivideByConst(mat.handle, ctypes.c_float(val), output.handle)
...
def conv2d(input_x, input_f, output, padding=0, stride=1):
    ...
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlConv2d(input_x.handle, input_f.handle, output.handle, ctypes.c_int(padding), ctypes.c_int(stride))
...
def concat(input_x, input_y, output, axis=0):
    ...
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlConcat(input_x.handle, input_y.handle, output.handle, ctypes.c_int(axis))
...
def concat_gradient(output_gradient, input_gradient, axis=0, id=0):
    ...
    _LIB.cpu_Concat_Gradient(output_gradient.handle, input_gradient.handle, ctypes.c_int(axis), ctypes.c_int(id))
...
def dropout_gradient(in_gradient_y, dropout, out_gradient_x):
    ...
    _LIB.cpu_Dropout_Gradient(in_gradient_y.handle, ctypes.c_float(dropout), out_gradient_x.handle)
...
def sgd_update_sparse(param, grad_ind, grad_value, lr):
    ...
    _LIB.cpu_SGDOptimizerSparseUpdate(param.handle, grad_ind.handle, grad_value.handle, ctypes.c_float(lr))
...
def normal_init(param, mean, stddev, seed):
    assert isinstance(param, _nd.NDArray)
    _LIB.cpu_NormalInit(param.handle, ctypes.c_float(mean), ctypes.c_float(stddev), ctypes.c_ulonglong(seed))


def uniform_init(param, lb, ub, seed):
    assert isinstance(param, _nd.NDArray)
    _LIB.cpu_UniformInit(param.handle, ctypes.c_float(lb), ctypes.c_float(ub), ctypes.c_ulonglong(seed))


def truncated_normal_init(param, mean, stddev, seed):
    assert isinstance(param, _nd.NDArray)
    _LIB.cpu_TruncatedNormalInit(param.handle, ctypes.c_float(mean), ctypes.c_float(stddev), ctypes.c_ulonglong(seed))
