...
class PipelineSendOp(Op):
    def __init__(self, node_A, destination, comm, stream=None, ctx=None):
        ...
        self.desc = self.name + '(send node %s to rank %s)' % (node_A.name, str(destination))
        self.shape = None
    ...
    def infer_shape(self, input_shapes):
        ...
        if not self.shape_is_sent:
            ...
            self.comm.dlarraySend(payload,
                                ...
                                self.const_attr,
                                self.comm_stream)
            # print("gpu{} send shape {} to gpu{}".format(
            #     self.ctx.device_id, payload.asnumpy(), self.const_attr))
        return shape
