...
class PadOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['cpu_Pad']:
                cpu_pad(input_vals[0], output_val, self.paddings, self.mode, constant_values=0)
            else:
                output_val[:] = pad_np(input_vals[0].asnumpy(), self.paddings, self.mode, constant_values=0)
        else:
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['cpu_Pad_Gradient']:
                cpu_pad_gradient(input_vals[0], output_val, self.paddings, self.mode)
            else:
                output_val[:] = pad_np_gradient(input_vals[0].asnumpy(), self.paddings)
        else:
            pad_gradient(input_vals[0], output_val, self.paddings, self.mode, stream_handle)
