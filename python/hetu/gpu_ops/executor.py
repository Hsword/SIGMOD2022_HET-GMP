...
from .SharedTable import SharedTableOp
...
def wrapped_mpi_nccl_init(init_nccl=True, devices=None):
    ...
    global nccl_comm
    global nccl_inited
    nccl_inited = init_nccl
    if 'nccl_comm' not in globals():
        nccl_comm = mpi_nccl_communicator(devices=devices)
        if init_nccl:
            nccl_comm.ncclInit()
    local_rank = nccl_comm.localRank.value
    device_id = nccl_comm.device_id.value
    rank = nccl_comm.myRank.value
    nrank = nccl_comm.nRanks.value
    return nccl_comm, local_rank, device_id, rank, nrank

def new_group_comm(group_list=None):
    from ..communicator.mpi_nccl_comm import ncclDataType_t, ncclRedOp_t, mpi_nccl_communicator
    if group_list is None:
        comm = mpi_nccl_communicator(mpi_init=False)
        comm.ncclInit()
    else:
        comm = mpi_nccl_communicator(mpi_init=False)
        comm.ncclGroupInit(group_list)
    return comm
...
def wrapped_mpi_nccl_finish():
    global nccl_comm
    global nccl_inited
    if 'nccl_comm' in globals():
        if 'nccl_inited' in globals() and nccl_inited:
            nccl_comm.ncclFinish()
        del nccl_comm
...
class HetuConfig(object):
    ...
    def __init__(
        ...
        if context_launch:
            # with context usage
            launchMPI, launchPS, self.node_strategy, devices = get_launch_config_by_traverse_nodes(eval_node_list, ctx)
            gpu_devices = sorted([dev.device_id for dev in devices if ndarray.is_gpu_ctx(dev)])
            ...
            # print('this is comm mode:', self.comm_mode)
        ...
        if self.comm_mode == 'PS' or self.comm_mode == 'Hybrid':
            ...
            self.rank = int(os.getenv('HEAPPROFILE').lstrip('./W'))
            self.nrank = int(os.environ['DMLC_NUM_WORKER']) if 'DMLC_NUM_WORKER' in os.environ else 1
        self.local_rank = self.rank
        if self.comm_mode == "Hybrid" or self.comm_mode == "AllReduce":
            self.nccl_comm, self.local_rank, device_id, allreduce_rank, self.nrank = wrapped_mpi_nccl_init(devices=gpu_devices)
        elif context_launch:
            self.nccl_comm, self.local_rank, device_id, allreduce_rank, self.nrank = wrapped_mpi_nccl_init(init_nccl=init_p2p_stream, devices=gpu_devices)
        if self.rank is None and (context_launch or self.comm_mode is not None):
            self.rank = allreduce_rank
        ...
        if context_launch:
            ...
            self.p2p_stream = create_stream_handle(self.context) if init_p2p_stream else None
            self.my_eval_nodes, trainable_params, has_send_recv = assign_context_by_traverse_nodes(eval_node_list, self.context, self.nccl_comm, self.p2p_stream)
            if (self.comm_mode == "Hybrid" or self.comm_mode == "AllReduce") and has_send_recv:
                ...
                temp_group_comms = {}
                for group in groups:
                    temp_group_comms[group] = new_group_comm([x.device_id for x in group])
                self.param_allreduce_group = {n: temp_group_comms[n.raw_ctx] for n in trainable_params}
        else:
            self.context = ctx
        # print(self.context)
        ...
        # define streams
        self.comp_stream = create_stream_handle(self.context) if on_gpu else None
        self.h2d_stream = create_stream_handle(self.context) if on_gpu else None
        self.d2h_stream = create_stream_handle(self.context) if on_gpu else None
...
class Executor(object):
    ...
    def __init__(self, eval_node_dict, config=None, **kargs):
        ...
        if config is None:
            config = HetuConfig(eval_node_list=all_eval_nodes, train_name=train_name, val_name=val_name, **kargs)
        assert isinstance(config, HetuConfig), 'Config type %s invalid.' % str(type(config))
        ...
        self.param_nodes = [node for node in self.topo_order if isinstance(node, PlaceholderOp) and node.trainable]
    ...
    @property
    def batch_num(self):
        assert len(self.subexecutor) == 1, 'Batch num should be used with only 1 subexecutor.'
        ...
        if self.comm_mode in (None, 'AllReduce'):
            ...
            for node in self.topo_order:
                if isinstance(node, PlaceholderOp) and node.trainable and not node.is_embed:
                    ...
                    self.ps_comm.SparsePull(nodeid, node.inputs[1].get_next_arr(name).handle, self.param_psval_map[node.inputs[0]].handle)
                    node.event.update()
            self.ps_comm.BarrierWorker()
...
class SubExecutor(object):
    ...
    def update_executor(self, eval_node_list):
        ...
        if config.p2p_stream and self.inference == True:
    ...
    def run(self, eval_node_list={}, feed_dict={}, convert_to_numpy_ret_vals=False):
        ...
        """
        assert len(feed_dict) == len(self.need_feed_nodes) or self.use_p2p, 'Feed dict invalid.'
        ...
        for node in self.computing_nodes:
            ...
            elif isinstance(node, (DropoutOp, Batch_NormalizationOp, Layer_NormalizationOp, SharedTableOp)):
...
def gradients(output_node, node_list, insert_grad=None):
    ...
    if isinstance(output_node, list):
        ...
        node_to_output_grads_list = {
            output_node: [OnesLike.oneslike_op(output_node)] if insert_grad is None else [insert_grad]
        }
    ...
    for node in reverse_topo_order:
        #output_grad = sum_node_list(node_to_output_grads_list[node], node.raw_ctx)
        # here the ctx for embedding lookup is a workaround
        # TODO: when implement PS strategy for context semantics, modify here
        if isinstance(node, EmbeddingLookUp) or isinstance(node, SharedTableOp):
            output_grad = sum_node_list(node_to_output_grads_list[node], node_to_output_grads_list[node][0].raw_ctx)
        else:
            output_grad = sum_node_list(node_to_output_grads_list[node], node.raw_ctx)
