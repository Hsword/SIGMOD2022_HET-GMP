...
class BinaryCrossEntropyOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            binary_cross_entropy(input_vals[0], input_vals[1], output_val, stream_handle)

    def gradient(self, output_grad):

        grad_A = binarycrossentropy_gradient_op(self.inputs[0], self.inputs[1], output_grad, ctx=self.raw_ctx)
        grad_B = None
...
class BinaryCrossEntropyGradientOp(Op):
    def __init__(self, prediction, label, output_grad_node, ctx=None):
        super().__init__(BinaryCrossEntropyGradientOp, [prediction, label, output_grad_node], ctx)
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            binary_cross_entropy_gradient(input_vals[0], input_vals[1], input_vals[2], output_val, stream_handle)
