    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlAvgPool']:
                cpu_avg_pool(input_vals[0], self.kernel_H, self.kernel_W, output_val, self.padding, self.stride)
            else:
...
class Avg_Pool2d_GradientOp(Op):
    def __init__(self, node_out, node_out_gradient, node_in, kernel_H, kernel_W, padding, stride, ctx=None):
        super().__init__(Avg_Pool2d_GradientOp, [node_out, node_out_gradient, node_in], ctx)
        self.padding = padding
        self.stride = stride
        self.kernel_H = kernel_H
        self.kernel_W = kernel_W
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlAvgPool_Gradient']:
                cpu_avg_pool_gradient(input_vals[1], self.kernel_H, self.kernel_W, output_val, self.padding, self.stride)
            else:
