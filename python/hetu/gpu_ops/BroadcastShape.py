...
class BroadcastShapeOp(Op):
        ...
        if self.on_cpu:
            ...
            for i in range(len(input_shape)):
                if self.add_axes and i in self.add_axes:
                    input_shape[i] = 1
            output_val[:] = np.broadcast_to(input_vals[0].asnumpy().reshape(input_shape), self.target_shape)
        else:
            if self.inplace:
                input_vals[0].broadcast_to(self.target_shape, output_val, self.add_axes)
            else:
                # broadcast_shape(input_vals[0], output_val, self.add_axes, stream_handle)
                broadcast_shape_simple(input_vals[0], output_val, self.out_strides, self.in_dims, stream_handle)

    def gradient(self, output_grad):
        self.grad_node = reduce_sum_op(output_grad, None, None, ctx=self.raw_ctx)
    ...
    def infer_shape(self, input_shapes):
        ...
        if self.on_gpu and not self.inplace:
            ...
            self.out_strides = ndarray.array(out_strides, self.ctx, data_type=np.int32)
