...
class PipelineReceiveOp(Op):
    def __init__(self, source, comm, stream=None, ctx=None):
        ...
        self.desc = self.name + '(%s receive from %s)' % (str(self.ctx.device_id) , str(source))
        self.shape = None
    ...
    def infer_shape(self, input_shapes):
        if not self.shape_is_received:
            ...
            self.comm.dlarrayRecv(shape_arr,
                                ...
                                self.const_attr,
                                self.comm_stream)
            # print("gpu{} receive shape {} from gpu{}".format(
            #     self.ctx.device_id, shape_arr.asnumpy(), self.const_attr))
