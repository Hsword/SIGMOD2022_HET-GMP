...
class DivOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        ...
        if self.on_cpu:
            if DNNL_LIB['DnnlMatrixElementwiseDivide']:
                cpu_matrix_elementwise_divide(input_vals[0], input_vals[1], output_val)
            else:
    ...
    def gradient(self, output_grad):
        ...
        divisor_grad = opposite_op(div_op(div_op(self.inputs[0], self.inputs[1], ctx=self.raw_ctx), self.inputs[1], ctx=self.raw_ctx))
...
class DivConstOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlMatrixElementwiseDivideByConst']:
                cpu_matrix_elementwise_divide_by_const(input_vals[0], self.const_attr, output_val)
            else:
    ...
    def gradient(self, output_grad):
        divisor_grad = div_op(div_const_op(-self.const_attr, self.inputs[0], ctx=self.raw_ctx), self.inputs[0], ctx=self.raw_ctx)
