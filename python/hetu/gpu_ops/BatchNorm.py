...
class Batch_NormalizationOp(Op):
    def __init__(self, node_in, bn_scale, bn_bias, momentum=0.99, eps=0.01, ctx=None):
        super().__init__(Batch_NormalizationOp, [node_in, bn_scale, bn_bias], ctx)
            ...
            if self.on_cpu:
                if DNNL_LIB['DnnlBatchNorm_Inference']:
                    ...
                    output_val[:] = batchnorm_inference(input_vals[0].asnumpy(), input_vals[1].asnumpy(),
                                                        input_vals[2].asnumpy(), self.save_mean, self.save_var,
                                                        self.eps)
            else:
                CuDNN_Batch_Normalization_inference(
                    input_vals[0], input_vals[1], input_vals[2], output_val, self.save_mean, self.save_var, self.eps,
                    stream_handle)
        else:
            if self.on_cpu:
                if DNNL_LIB['DnnlBatchNorm']:
                    if self.save_mean is None:
                        ...
                        self.save_mean = ndarray.array(np.zeros([C], dtype=np.float32), ctx=ndarray.cpu(dev_id))
                        self.save_var = ndarray.array(np.zeros([C], dtype=np.float32), ctx=ndarray.cpu(dev_id))
                ...
                if self.save_mean == None:
                    dev_id = input_vals[0].handle.contents.ctx.device_id
                    C = input_vals[0].shape[1]
                    self.save_mean = ndarray.array(np.zeros([1, C, 1, 1]), ctx=ndarray.gpu(dev_id))
                    self.save_var = ndarray.array(np.zeros([1, C, 1, 1]), ctx=ndarray.gpu(dev_id))
...
class Batch_Normalization_GradientOp(Op):
    def __init__(self, out_gradient, in_node, bn_scale, forward_node, eps, ctx=None):
        super().__init__(Batch_Normalization_GradientOp, [out_gradient, in_node, bn_scale], ctx)
        ...
        if self.on_cpu:
            if DNNL_LIB['DnnlBatchNorm_Gradient']:
                if self.tmp_gradient_bn_bias is None:
                    ...
                    self.tmp_gradient_bn_bias = np.zeros(shape=shapebn, dtype=np.float32)
                    self.tmp_gradient_bn_scale = np.zeros(shape=shapebn, dtype=np.float32)
                    self.tmp_gradient_in_arr = np.zeros(shape=input_vals[1].shape, dtype=np.float32)
                ...
                if self.tmp_gradient_bn_bias is None:
                    ...
                    self.tmp_gradient_bn_bias = np.zeros(shape=shapebn, dtype=typebn)
                    self.tmp_gradient_bn_scale = np.zeros(shape=shapebn, dtype=typebn)
                self.tmp_gradient_in_arr, self.tmp_gradient_bn_scale, self.tmp_gradient_bn_bias = batchnorm_backward(
                    input_vals[0].asnumpy(), input_vals[1].asnumpy(), input_vals[2].asnumpy(),
...
class Batch_Normalization_Gradient_of_DataOp(Op):
    def __init__(self, bn_gradient, in_arr, ctx=None):
        super().__init__(Batch_Normalization_Gradient_of_DataOp, [bn_gradient, in_arr], ctx)
...
class Batch_Normalization_Gradient_of_ScaleOp(Op):
    def __init__(self, bn_gradient, in_scale, ctx=None):
        super().__init__(Batch_Normalization_Gradient_of_ScaleOp, [bn_gradient, in_scale], ctx)
...
class Batch_Normalization_Gradient_of_BiasOp(Op):
    def __init__(self, bn_gradient, in_bias, ctx=None):
        super().__init__(Batch_Normalization_Gradient_of_BiasOp, [bn_gradient, in_bias], ctx)
