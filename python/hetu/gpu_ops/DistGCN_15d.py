...
def broad_func(node_count, adj_matrix, inputs, rank, size, replication, row_groups, col_groups, ctx, comm=None, stream_handle=None):
    ...
    proc_node_count = row_num(node_count, rank//replication, size // replication)
    ...
    for i in range(stages):
        ...
        if q_c == size // replication - 1:
            inputs_recv = ndarray.empty((row_num(node_count, size//replication - 1, size//replication), inputs.shape[1]), ctx=ctx)
        if q == rank:
            inputs.copyto(inputs_recv)
        
        from ..communicator.mpi_nccl_comm import ncclDataType_t, ncclRedOp_t, mpi_nccl_communicator        
        if replication > 1:       
            col_groups[rank_col].dlarrayBroadcast(inputs_recv, inputs_recv, ncclDataType_t.ncclFloat32, q)
        else:
            comm.dlarrayBroadcast(inputs_recv, inputs_recv, ncclDataType_t.ncclFloat32, q)
                
        from ..gpu_links import CuSparse_Csrmm, matrix_elementwise_add     
        CuSparse_Csrmm(adj_matrix, False, inputs_recv, False, tmp, stream=stream_handle, start_pos=int(start_pos[i]), end_pos=int(end_pos[i]))
        matrix_elementwise_add(z_loc, tmp, z_loc, stream_handle)
          
    if replication > 1:
        row_groups[rank_c].dlarrayNcclAllReduce(z_loc, z_loc, ncclDataType_t.ncclFloat32, reduceop=ncclRedOp_t.ncclSum)
...
class DistGCN_15dOp(Op):
    def __init__(self, node_A, node_B, node_C, node_Count_Self, node_Count_All, size, replication, device_id, comm, comm_groups=[None,None], need_W=True):
        super().__init__(DistGCN_15dOp, [node_A, node_B, node_C], ctx=ndarray.gpu(device_id))
        self.need_W = need_W
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        ...
        if weight.shape[1]<inputs_H.shape[1]:
            ...
            if (self.need_W == True):
                ...
                matrix_multiply(inputs_H, False, weight, False, HW, stream_handle)
            else:
                HW = inputs_H
            z = broad_func(node_count, adj_matrix, HW, rank, self.size, self.replication, row_groups=self.comm_groups[0], col_groups=self.comm_groups[1], ctx=ctx, comm=comm, stream_handle=stream_handle)
            z.copyto(output_val)
        else:
            AH = broad_func(node_count, adj_matrix, inputs_H, rank, self.size, self.replication, row_groups=self.comm_groups[0], col_groups=self.comm_groups[1], ctx=ctx, comm=comm, stream_handle=stream_handle)
    ...
    def gradient(self, output_grad):
        ...
        comm = self.comm
        rank = comm.localRank.value
        ag = distgcn_15d_op(adj_matrix, output_grad, weight, node_Count_Self, node_Count_All, self.size, self.replication, self.device_id, comm, self.comm_groups, need_W=False)
        ...
        if self.replication > 1:
            ...
            grad_W = groupallreduceCommunicate_op(grad_weight, weight_groups[rank % self.replication])
        else:
