...
class AddOp(Op):
    ...
    def _compute_with_index(self, input_vals, output_val, stream_handle=None):
        def cpu_oneside_add(sparse, dense):
            ...
            dense[sparse.indices.asnumpy().astype(np.int)] += sparse.values.asnumpy()
        ...
        if self.on_cpu:
            ...
            if first_indexed and not second_indexed:
                ...
                indexedslice_oneside_add(input_vals[0], output_val, stream_handle)
            elif not first_indexed and second_indexed:
                input_vals[0].copyto(output_val)
                indexedslice_oneside_add(input_vals[1], output_val, stream_handle)
            elif first_indexed and second_indexed:
    ...
    def _compute_on_cpu_simple(self, input_vals, output_val, stream_handle=None):
        ...
        if DNNL_LIB['DnnlMatrixElementwiseAdd'] and input_vals[0].shape == input_vals[1].shape:
            cpu_matrix_elementwise_add(input_vals[0], input_vals[1], output_val)
            ...
            if input_vals[1].shape == (1,):
                ...
                cpu_matrix_elementwise_add_by_const(input_vals[0], const_val, output_val)
            elif input_vals[0].shape == (1,):
                const_val = input_vals[0].asnumpy()[0]
                cpu_matrix_elementwise_add_by_const(input_vals[1], const_val, output_val)
        else:
    ...
    def _reset_gpu_buffer(self, input_val1, input_val2, output_val):
        if self.check_reset:
            strides = list(input_val1.stride) + list(input_val2.stride) + list(output_val.stride)
            self.gpu_buffer = ndarray.array(strides, self.ctx, data_type=np.uintc)        
        ...
        if no_broadcast:
            ...
            for i in range(len(short_shapes)):
                if short_shapes[i]!=1 and short_shapes[i]!=long_shapes[len(long_shapes)-len(short_shapes) + i]:
                    assert False,"can't add variables of shapes  "+str(input_shapes[0])+str(input_shapes[1])
            output = long_shapes
