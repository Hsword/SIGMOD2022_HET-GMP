...
class ParameterServerCommunicateOp(Op):
    ...
    def __init__(self, nodeA, parameter, optimizer):
        ...
        self.learning_rate = -optimizer[1][0] / int(os.environ['DMLC_NUM_WORKER'])
    ...
    def forward_hook(self, config):
        ...
        if config.cstable_policy is not None and self.parameter.is_embed:
            ...
            # ALREADY has barrier!
            self.parameter.initializer.init_on_ps(self.comm, self.ps_id, 2, seed=config.seed + self.ps_id.value, opt=self.optimizer)
            self.cache = CacheSparseTable(limit, node_shape[0], node_shape[1], self.parameter.id, config.cstable_policy, config.cache_bound)
            ...
            if config.prefetch:
                ...
                self.sparse_pull_val = ndarray.empty(tuple(local_shape), ctx=ndarray.cpu(0))
                self.parameter.event.update_ts(self.cache.embedding_lookup(self.dl_node.get_next_arr(self.dl_name), self.sparse_pull_val))
        ...
        # this function synchronously initialize meta information and do the initialization,
        # ALREADY has barrier!
        self.parameter.initializer.init_on_ps(self.comm, self.ps_id, int(self.parameter.is_embed), seed=config.seed + self.ps_id.value, opt=self.optimizer)
        if self_sparse:
            if config.prefetch:
                self.dl_name = config.train_name
                self.dl_node = self.inputs[0].inputs[1]
                ...
                local_shape.append(node_shape[-1])
                self.sparse_pull_val = ndarray.empty(tuple(local_shape), ctx=ndarray.cpu(0))
                self.comm.SparsePull(self.ps_id, self.dl_node.get_next_arr(self.dl_name).handle, self.sparse_pull_val.handle)
                config.ps_map[self.parameter] = self.sparse_pull_val
                self.parameter.event.update()
        else:
            ...
            self.parameter.tensor_value = self.pull_val
            self.parameter.event.update()
...
class ParameterServerSparsePullOp(Op):
    def __init__(self, node, deps_node):
        super().__init__(ParameterServerSparsePullOp, [node] + deps_node, node.ctx)
        self.on_gpu = ndarray.is_gpu_ctx(self.ctx)
        self.on_cpu = not self.on_gpu
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        ...
        if self.use_cache_table:
            ts = self.cache.embedding_lookup(self.dl_node.get_next_arr(self.dl_name), self.sparse_pull_val)
            self.parameter.event.update_ts(ts)
            return
        assert self.on_cpu == True
        assert isinstance(input_vals[0], ndarray.NDArray)
        comm.SparsePull(self.ps_id, self.dl_node.get_next_arr(self.dl_name).handle, self.sparse_pull_val.handle)
        self.parameter.event.update()
    ...
    def forward_hook(self, config):
        ...
        node_shape = self.parameter.shape
        assert (config.use_sparse_pull or self.use_cache_table) and self.parameter.is_embed
