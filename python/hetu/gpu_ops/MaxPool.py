    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlMaxPool']:
                cpu_max_pooling(input_vals[0],self.kernel_H, self.kernel_W, output_val, self.padding, self.stride)
            else:
                output_val[:] = np_max_pooling(input_vals[0].asnumpy(), self.kernel_H, self.kernel_W, self.padding, self.stride)
        else:
            CuDNN_max_pooling2d(
...
class Max_Pool2d_GradientOp(Op):
    def __init__(self, node_out, node_out_gradient, node_in, kernel_H, kernel_W, padding, stride, ctx=None):
        super().__init__(Max_Pool2d_GradientOp, [node_out, node_out_gradient, node_in], ctx)
        self.padding = padding
        self.stride = stride
        self.kernel_H = kernel_H
        self.kernel_W = kernel_W
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlMaxPool_Gradient']:
                cpu_max_pooling_gradient(input_vals[2], input_vals[1], self.kernel_H, self.kernel_W, output_val, self.padding, self.stride)
            else:
                output_val[:] = np_max_pooling_gradient(input_vals[2].asnumpy(), input_vals[1].asnumpy(), self.kernel_H, self.kernel_W, self.padding, self.stride)
        else:
