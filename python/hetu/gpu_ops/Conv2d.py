    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlConv2d']:
                cpu_conv2d(input_vals[0], input_vals[1], output_val, self.padding, self.stride)
            else:
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlConv2d_Gradient_of_Data']:
                cpu_conv2d_gradient_of_data(input_vals[0], input_vals[1], output_val, self.padding, self.stride)
            else:
...
class Conv2d_Gradient_of_FilterOp(Op):
    ...
    def __init__(self, input_X, gradient_Y, padding=0, stride=1, ctx=None):
        super().__init__(Conv2d_Gradient_of_FilterOp, [input_X, gradient_Y], ctx)
        self.padding = padding
        self.stride = stride
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            if DNNL_LIB['DnnlConv2d_Gradient_of_Filter']:
                cpu_conv2d_gradient_of_filter(input_vals[0], input_vals[1], output_val, self.padding, self.stride)
            else:
