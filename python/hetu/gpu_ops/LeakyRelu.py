...
class LeakyReluOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            """
            if DNNL_LIB['DnnlLeakyRelu']:
                cpu_leaky_relu(input_vals[0], output_val)
            else:
                output_val[:] = np.maximum(input_vals[0].asnumpy(), 0)
            """
        else:
            leaky_relu(input_vals[0], self.const_attr, output_val, stream_handle)
...
class LeakyReluGradientOp(Op):
    def __init__(self, node_A, node_B, const_val, ctx=None):
        ...
        self.const_attr = const_val
        self.desc = self.name + '(%s, %s, %s)' % (node_A.name, node_B.name, str(const_val))

    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            raise NotImplementedError
            """
            if DNNL_LIB['DnnlLeakyRelu_Gradient']:
                cpu_leaky_relu_gradient(input_vals[0], input_vals[1], output_val)
            # heaviside function, 0.5 at x=0
            else:
                output_val[:] = (np.sign(input_vals[0].asnumpy()) + 1) * 0.5 * input_vals[1].asnumpy()
            """
        else:
            leaky_relu_gradient(input_vals[0], input_vals[1], self.const_attr, output_val, stream_handle)
