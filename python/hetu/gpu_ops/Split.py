...
class SplitOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            index = tuple([slice(i, i+j) for i, j in zip(self.begin_pos, self.output_shape)])
            output_val[:] = input_vals[0].asnumpy()[index]
        else:
            # matrix_slice(input_vals[0], output_val, self.begin_pos, stream_handle)
            matrix_slice_simple(input_vals[0], output_val, self.gpu_buffer, stream_handle)
    
    def gradient(self, output_grad):
        self.grad_node = split_gradient_op(output_grad, self.axes, self.indices, self.splits, ctx=self.raw_ctx)
    ...
    def infer_shape(self, input_shapes):
        ...
        for axe, ind, spl in zip(self.axes, self.indices, self.splits):
            ...
            self.output_shape[axe] = part_size if ind != spl - 1 else ori_shape[axe] - self.begin_pos[axe]
        ...
        if self.on_gpu:
            ...
            self.gpu_buffer = ndarray.array(gpu_buf, self.ctx, data_type=np.uintc)
...
class SplitGradientOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            index = tuple([slice(i, i+j) for i, j in zip(self.begin_pos, ori_shape)])
            output_val[index] = input_vals[0]
        else:
            # matrix_slice_gradient(input_vals[0], output_val, self.begin_pos, stream_handle)
            matrix_slice_gradient_simple(input_vals[0], output_val, self.gpu_buffer, stream_handle)
