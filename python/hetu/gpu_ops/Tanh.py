...
class TanhOp(Op):
    ...
    def gradient(self, output_grad):
        grad_A=1 + -1*tanh_op(self.inputs[0], ctx=self.raw_ctx)*tanh_op(self.inputs[0], ctx=self.raw_ctx)
        return [grad_A*output_grad]
        #return [(1-tanh_op(self.inputs[0])*tanh_op(self.inputs[0]))*output_grad]
