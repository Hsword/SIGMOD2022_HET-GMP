...
class AllReduceCommunicateOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle = None):
        if self.on_cpu:
            ...
            self.comm.dlarrayNcclAllReduce(input_vals[0], output_val, self.dtype, self.reduce_op)
        else:
            if self.event == None:
                self.event = create_event_handle(input_vals[0].ctx)
            if isinstance(input_vals[0], ndarray.NDArray):
                self.comm.dlarrayNcclAllReduce(input_vals[0], output_val, self.dtype, self.reduce_op, stream_handle)
                self.event.record(stream_handle)
            elif isinstance(input_vals[0], ndarray.IndexedSlices):
                # ?should use allgather?
                self.comm.dlarrayNcclAllReduce(input_vals[0].indices, output_val.indices, self.dtype, self.reduce_op, stream_handle)
                self.comm.dlarrayNcclAllReduce(input_vals[0].values, output_val.values, self.dtype, self.reduce_op, stream_handle)
                self.event.record(stream_handle)
...
class GroupAllReduceCommunicateOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle = None):
        ...
        self.group_comm.dlarrayNcclAllReduce(output_val, output_val, ncclDataType_t.ncclFloat32, ncclRedOp_t.ncclSum, stream_handle)
