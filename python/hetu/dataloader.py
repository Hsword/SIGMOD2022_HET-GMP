...
class Dataloader(object):
    def __init__(self, raw_data, batch_size, name='default', func=None, drop_last=True, use_numpy=False):
        self.func = func if func else lambda x: x
        self.raw_data = np.array(self.func(raw_data))
    ...
    def init_states(self, rank=None, nrank=None):
        ...
        for i in range(self.queue_size):
            ...
            if self.use_numpy:
                 self.arrs.append(self.raw_data[self.index:next_index])
            else:
                self.arrs.append(ndarray.array(
                    self.raw_data[self.index:next_index], ctx=ndarray.cpu(0)))
        ...
        if not self.drop_last:
            ...
            if res_num > 0:
                self.arrs.append(ndarray.empty(tuple([res_num] + list(self.shape[1:])), ctx=ndarray.cpu(0)))
    ...
    def _get_arr(self, batchind):
        ...
        if batchind == self.max_key:
            ...
            if next_index <= self.samples_num:
                ...
                if self.use_numpy:
                    self.arrs[temp_ind] = self.raw_data[self.index:next_index]
                self.arrs[temp_ind][:] = self.raw_data[self.index:next_index]
            else:
                assert not self.drop_last
                if self.use_numpy:
                    self.arrs[-1] = self.raw_data[self.index:next_index]
                self.arrs[-1][:] = self.raw_data[self.index:next_index]
...
class DataloaderOp(Op):
    def __init__(self, dataloaders):
        ...
        self.name = "DataloaderOp%d(%s)" % (self.id, '_'.join(self.dataloaders.keys()))
        self.desc = self.name
    ...
    def forward_hook(self, config):
        # assert config.train_name is None or config.train_name in self.dataloaders.keys(), \
        #     'Executor uses %s dataloader, not included in (%s).' % (config.train_name, ', '.join(self.dataloaders.keys()))
        # assert config.val_name is None or config.val_name in self.dataloaders.keys(), \
        #     'Executor uses %s dataloader, not included in (%s).' % (config.val_name, ', '.join(self.dataloaders.keys()))
        pass
