...
class CacheSparseTable:
    ...
    def embedding_lookup(self, keys, dest, sync=False):
        ...
        if type(keys) is np.ndarray and type(dest) is np.ndarray:
            ...
            wait = self.cache.embedding_lookup_raw(keys.handle.contents.data, dest.handle.contents.data, np.prod(keys.shape))
        else:
            raise TypeError
        if sync:
            wait.wait()
    ...
    def embedding_update(self, keys, grads, sync=False):
        ...
        if type(keys) is np.ndarray and type(grads) is np.ndarray:
            ...
            wait = self.cache.embedding_update_raw(keys.handle.contents.data, grads.handle.contents.data, np.prod(keys.shape))
        else:
            raise TypeError
        if sync:
            wait.wait()
    ...
    def embedding_push_pull(self, pullkeys, dest, pushkeys, grads, sync=False):
        ...
        if type(pullkeys) is ndarray.NDArray and type(dest) is ndarray.NDArray and \
            ...
            wait = self.cache.embedding_push_pull_raw(
                pullkeys.handle.contents.data, dest.handle.contents.data, np.prod(pullkeys.shape),
                pushkeys.handle.contents.data, grads.handle.contents.data, np.prod(pushkeys.shape)
            )
        else:
            raise TypeError
        if sync:
