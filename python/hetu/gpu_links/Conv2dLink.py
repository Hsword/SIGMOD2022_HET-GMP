...
# def conv2d_transpose(in_arr_x, in_arr_f, in_gradient_y, out_gradient_x, out_gradient_f, workspace_im2col, workspace_batch_filter, padding = 0, stride = 1, stream = None):
#     assert isinstance(in_arr_x, _nd.NDArray)
#     assert isinstance(in_arr_f, _nd.NDArray)
...
#     _LIB.DLGpuConv2d_transpose(in_arr_x.handle, in_arr_f.handle, in_gradient_y.handle, out_gradient_x.handle, out_gradient_f.handle, workspace_im2col.handle , workspace_batch_filter.handle, padding, stride, stream.handle if stream else None)
