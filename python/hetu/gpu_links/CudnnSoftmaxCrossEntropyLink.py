...
def CuDNN_softmax_cross_entropy(y, y_, out, stream = None):
    ...
    _LIB.CuDNN_DLGpuSoftmaxEntropy(y.handle, y_.handle, out.handle, stream.handle if stream else None)
...
def CuDNN_softmax_cross_entropy_gradient(grad_arr, y_arr, label, out_arr, stream=None):
    ...
    _LIB.CuDNN_DLGpuSoftmaxEntropyGradient(grad_arr.handle, y_arr.handle, label.handle, out_arr.handle, stream.handle if stream else None)
