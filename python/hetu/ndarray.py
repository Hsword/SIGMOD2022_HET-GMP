...
class NDArray(object):
    ...
    def _async_copyfrom(self, source_array, stream_handle, event_handle=None):
        ...
        if not event_handle is None:
            check_call(_LIB.DLEventRecord(stream_handle.handle, event_handle.handle))
    ...
    def lazy_callback(self, stream = None):
        ...
        handle = DLArrayHandle()
        check_call(_LIB.DLArrayAlloc(shape, stride, ndim, self.handle.contents.ctx, ctypes.byref(handle)))
        check_call(_LIB.DLGpuArrayLazyCallback(self.handle, handle, stream.handle if stream else None))
        self.handle = handle
    ...
    # def allreduce2(self, target):
    #     check_call(_LIB.DLArrayAllReduce2(self.handle, target.handle, None))
    #     return self
    
    # def allreduce4(self, t1, t2, t3):
    #     check_call(_LIB.DLArrayAllReduce4(self.handle, t1.handle, t2.handle, t3.handle, None))
    #     return self
   
    # def allreduce8(self, t1, t2, t3, t4, t5, t6, t7):
    #     check_call(_LIB.DLArrayAllReduce4(self.handle, t1.handle, t2.handle, t3.handle, t4.handle, t5.handle, t6.handle, t7.handle, None))
    #     return self
...
class IndexedSlices(object):
    ...
    def deduplicate(self, stream):
        ...
        _LIB.DLGpuArraySet(new_values.handle, ctypes.c_float(0), stream.handle if stream else None)
        _LIB.DeduplicateIndexedSlices(self.values.handle, inverse_on_ctx.handle, new_values.handle, stream.handle if stream else None)
    ...
    def cpu_deduplicate(self):
        ...
        # print(inverse.shape, inverse)
        # print(unique_indices.shape, unique_indices)
        # print(np_indices.shape, np_indices)
        # exit()
