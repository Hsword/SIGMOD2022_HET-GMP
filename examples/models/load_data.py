...
def process_dense_feats(data, feats):
    ...
    for f in feats:
        ...
        # d[f] = d[f].apply(lambda x: x+1 if x > 0 else 0)
    return d
...
def process_head_criteo_data(path=os.path.join(os.path.split(os.path.abspath(__file__))[0], '../datasets/criteo'), nrows=20000, return_val=True):
    ...
    sparse_feats = np.array(process_sparse_feats(df, sparse_feats)).astype(np.int32)
    if return_val:
        test_num = nrows // 10
...
def load_adult_data(return_val = True):
    ...
    x_train_wide, y_train_wide, x_test_wide, y_test_wide = wide(df_train, df_test, wide_cols, x_cols, target)
